...
void threadsafe_filework::push(const std::string& msg) {
    {   std::lock_guard<std::mutex> guard(_mtx);
        ...
        if(!fout.is_open()) throw std::ofstream::failure("Error opening file: push");
        fout << msg;
    }
...
std::string threadsafe_filework::wait_and_pop() {
    ...
    if(!fin.is_open()) throw std::ifstream::failure("Error opening file: wait_and_pop");
    std::getline(fin, res);
    std::ofstream fout(_file_name, std::ofstream::trunc);
    if(!fout.is_open()) throw std::ifstream::failure("Error opening trunc file: wait_and_pop");
    return res;
}

bool threadsafe_filework::empty() const {
    std::ifstream file(_file_name);
    if(!file.is_open()) throw std::ifstream::failure("Error opening file: empty");
